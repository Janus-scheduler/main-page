...
export default function ApiReferencePage() {
  ...
  const copyTag = () => {
    if (navigator.clipboard) navigator.clipboard.writeText('<janus-timeline></janus-timeline>').catch(() => {});
    setCopied(true);
  ...
  return (
    <DocsLayout currentPath="/docs/api/janus-timeline" toc={toc}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', font: '400 12px/1 "JetBrains Mono", monospace', color: 'var(--janus-text-muted)' }}>
        <a href="#" style={{ color: 'var(--janus-text-muted)' }}>Docs</a><span>/</span><a href="#" style={{ color: 'var(--janus-text-muted)' }}>API reference</a><span>/</span><a href="#" style={{ color: 'var(--janus-text-muted)' }}>Components</a><span>/</span><span style={{ color: 'var(--janus-text-secondary)' }}>janus-timeline</span>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '16px' }}>
        <h1 style={{ margin: 0, font: '700 36px/1.1 "JetBrains Mono", monospace', letterSpacing: '-0.02em', color: 'var(--janus-text)' }}>&lt;janus-timeline&gt;</h1>
      ...
      <p style={{ margin: '18px 0 0', maxWidth: '70ch', font: '400 16px/1.65 "DM Sans", sans-serif', color: 'var(--janus-text-secondary)', textWrap: 'pretty' }}>
        Renders resources as rows against a horizontal time axis, virtualising both directions. Owns drag, resize, zoom and conflict computation; reads its data from properties and reports changes as composed DOM events.
      </p>
      ...
      <div id="preview" style={{ marginTop: '26px', border: '1.5px solid var(--janus-border)', borderRadius: '14px', overflow: 'hidden', boxShadow: 'var(--janus-shadow)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--janus-surface)', borderBottom: '1.5px solid var(--janus-border)' }}>
          ...
          <span style={{ font: '500 11px/1 "JetBrains Mono", monospace', color: 'var(--janus-text-muted)' }}>&lt;janus-timeline&gt;</span>
        </div>
        <div style={{ padding: '16px', background: 'var(--janus-bg)' }}>
          <div style={{ border: '1.5px solid var(--janus-border)', borderRadius: '9px', overflow: 'hidden', height: '220px', background: '#ffffff', display: 'flex', alignItems: 'flex-start' }}>
            <img 
              src="/janus-timeline-preview.png" 
              alt="Janus Timeline Component Preview" 
              style={{ 
                width: '100%', 
                height: '100%',
                objectFit: 'cover',
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderTop: '1px solid var(--janus-border)', background: 'var(--janus-bg)' }}>
          <span style={{ font: '400 11.5px/1.5 "JetBrains Mono", monospace', color: 'var(--janus-text-muted)' }}>view="timeline" · zoom="hour" · snap-minutes="15"</span>
          <div style={{ display: 'flex', gap: '7px' }}>
            <span style={{ padding: '3px 9px', borderRadius: '999px', background: 'var(--janus-surface)', color: 'var(--janus-text-secondary)', font: '500 11px/1.35 "JetBrains Mono", monospace' }}>7 resources</span>
            <span style={{ padding: '3px 9px', borderRadius: '999px', background: 'var(--janus-surface)', color: 'var(--janus-text-secondary)', font: '500 11px/1.35 "JetBrains Mono", monospace' }}>multi-resource sync</span>
          </div>
        </div>
      </div>
      ...
      <div id="properties" style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', margin: '48px 0 0' }}>
        ...
        <span style={{ font: '400 11.5px/1 "JetBrains Mono", monospace', color: 'var(--janus-text-muted)' }}>10 of 10 · attribute names are kebab-case</span>
      </div>
      <div className="jscroll" style={{ marginTop: '14px', border: '1.5px solid var(--janus-border)', borderRadius: '14px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '170px 224px 118px 88px minmax(300px, 1fr)', minWidth: '100%' }}>
          ...
          <PropRow name="resources" type="Resource[]" def="—" req="yes" desc="Rows, in display order. Ids must be stable across renders." />
          <PropRow name="events" type="JanusEvent[]" def="—" req="yes" desc="Expanded instances. Pass a new array reference to update." />
          <PropRow name="assignments" type="Assignment[]" def="[]" req="no" desc="Event-to-resource joins. Omit for a one-row-per-event timeline." />
          <PropRow name="timeZone" type="string" def="system" req="no" desc="IANA zone used for display. Storage is always UTC." />
          <PropRow name="zoom" type="'15m'|'hour'|'day'|'week'" def="'hour'" req="no" desc="Width of one axis column. Changing it preserves the centred instant." />
          <PropRow name="snapMinutes" type="number" def="15" req="no" desc="Drag and resize granularity. Set to 0 for free positioning." />
        </div>
      </div>
      ...
      <div id="limitation" style={{ marginTop: '32px', border: '1.5px dashed var(--janus-text-muted)', borderRadius: '14px', background: '#18181b', padding: '18px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        <p style={{ margin: 0, maxWidth: '76ch', font: '400 13.5px/1.6 "DM Sans", sans-serif', color: '#e4e4e7' }}>
          <span style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '13px' }}>&lt;janus-timeline&gt;</span> virtualises rows, so it cannot be printed or exported to PDF directly — only the visible window exists in the DOM. Use <span style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '13px' }}>SchedulerManager.snapshot()</span> to render a full static grid for export. We do not plan to change this: keeping the whole range in the DOM would cost the performance the component exists for.
        </p>
      </div>
      ...
      <div style={{ marginTop: '44px', paddingTop: '22px', borderTop: '1.5px solid var(--janus-border)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
        <Link to="/docs/frameworks/vue" style={{ border: '1.5px solid var(--janus-border)', borderRadius: '14px', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
        ...
        <Link to="/docs/api/janus-month" style={{ border: '1.5px solid var(--janus-border)', borderRadius: '14px', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: '5px', alignItems: 'flex-end', textAlign: 'right' }}>
          <span style={{ font: '400 11px/1 "JetBrains Mono", monospace', color: 'var(--janus-text-muted)' }}>Next →</span>
          <span style={{ font: '600 15px/1.3 "JetBrains Mono", monospace', color: 'var(--janus-text)' }}>&lt;janus-month&gt;</span>
        </Link>
      </div>
    </DocsLayout>
  );
